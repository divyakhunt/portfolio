...
import {
  ...
  FaBolt, FaCuttlefish, FaChartBar, FaChartPie, FaFileAlt, FaMusic, FaFileAudio,
  FaWaveSquare, FaVolumeUp, FaCommentDots, FaHandSparkles, FaCog,
  FaChevronDown, FaBroadcastTower, FaHandsHelping, FaKeyboard, FaImages, FaSmile, FaBoxOpen
} from 'react-icons/fa';
import {
  SiC, SiCplusplus, SiScikitlearn, SiTensorflow, SiKeras, SiPytorch, SiOpencv, SiPandas,
  SiNumpy, SiStreamlit, SiFlask, SiJupyter, SiGooglecolab, SiKaggle,
  SiMysql, SiSpacy,
} from 'react-icons/si';

// NEW component to render project visuals based on an identifier
export const GetProjectVisual = ({ imageIdentifier, title, iconClassName }: { imageIdentifier?: string, title: string, iconClassName: string }) => {
  switch (imageIdentifier) {
    case 'speech_emotion':
      return <FaBroadcastTower className={iconClassName} aria-label={title} />;
    case 'sign_language':
      return <FaHandsHelping className={iconClassName} aria-label={title} />;
    case 'next_word':
      return <FaKeyboard className={iconClassName} aria-label={title} />;
    case 'image_captioning':
      return <FaImages className={iconClassName} aria-label={title} />;
    case 'facial_emotion':
      return <FaSmile className={iconClassName} aria-label={title} />;
    case "Ecomly":
      return (
    ...
    default:
      return <FaCog className={iconClassName} aria-label={title} />;
  }
};
...
const SkillCard = ({ item, large = true }: {item: {name: string, icon: React.ReactElement}, large?: boolean}) => {
...
export default function SkillsComponent() {
  ...
  useEffect(() => {
    const scrollRow1 = () => { if (!pausedRow1 && row1Ref.current) { row1Ref.current.scrollLeft += 0.5; if (row1Ref.current.scrollLeft >= row1Ref.current.scrollWidth / 2) row1Ref.current.scrollLeft = 0; } raf1.current = window.requestAnimationFrame(scrollRow1); };
    const scrollRow2 = () => { if (!pausedRow2 && row2Ref.current) { row2Ref.current.scrollLeft -= 0.5; if (row2Ref.current.scrollLeft <= 0) row2Ref.current.scrollLeft = row2Ref.current.scrollWidth / 2; } raf2.current = window.requestAnimationFrame(scrollRow2); };
    if (!expanded) { raf1.current = window.requestAnimationFrame(scrollRow1); raf2.current = window.requestAnimationFrame(scrollRow2); }
    return () => { 
  ...
  // --- NEW: Variants for the content flip animation (removed explicit :Variants type) ---
  const flipVariants = {
    initial: {
      rotateY: -90,
      opacity: 0,
    ...
    animate: {
      rotateY: 0,
      opacity: 1,
      transition: { type: 'spring' as const, stiffness: 150, damping: 25, delay: 0.1 }
    },
    exit: {
      rotateY: 90,
      opacity: 0,
  ...
  const gridItemVariants = {
    ...
    animate: { y: 0, opacity: 1, scale: 1, transition: { type: 'spring' as const, stiffness: 300, damping: 20 } }
  };
